...
def plot_rmse(folds: list[dict]):
    ...
    axis.bar(position - 0.2, pick("eval_calibrated_free_st_rmse_mm"), 0.4, color="tab:green",
    ...
    axis.plot(position, [float(fold["train_rmse_mm"]) for fold in folds], "^:", color="tab:orange",
...
def plot_parameter_spread(folds: list[dict]):
    ...
    figure.suptitle("Calibrated parameters across the 8 folds (deviation from their mean)", fontsize=14, fontweight="bold")
...
def plot_step_drift(folds: list[dict]):
    ...
    series = {
        ...
        "humeral head centre": np.linalg.norm(np.array([fold["step3_head"] - fold["step2_head"] for fold in folds]), axis=1),
